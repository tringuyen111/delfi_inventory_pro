...
const jsonInput = `[
  {
    "id": "NYC-CEN-OLD", "wh_code": "NYC-CEN", "wh_name": "New York Central Warehouse",
    "branch_code": "DELFI-HCM", "status": "Active"
  },
  {
    "id": "LDN-SUB-OLD", "wh_code": "LDN-SUB", "wh_name": "London Sub Warehouse",
    "branch_code": "DELFI-HN", "status": "Active"
  },
  {
    "id": "MIA-MAIN-OLD", "wh_code": "MIA-MAIN", "wh_name": "Miami Main Hub",
    "branch_code": "FASH-DN", "status": "Active"
  }
]`;

const branchIdMap = {
    "DELFI-HCM": "19131cba-8a63-40de-8fda-345924c69fcb",
    "DELFI-HN": "2b793261-de74-4a80-a98d-6f105196948d",
    "FASH-DN": "746f9111-d587-4051-8fcf-536562c7a761"
};

const DataSeederWarehouses: React.FC = () => {
    const [copied, setCopied] = useState(false);

    const csvOutput = useMemo(() => {
        const jsonData = JSON.parse(jsonInput);
        
        const headers = ["wh_code", "wh_name", "status", "branch_id"];
        
        const csvRows = jsonData.map((row: any) => {
            const branch_id = branchIdMap[row.branch_code as keyof typeof branchIdMap] || '';
            const values = [
                row.wh_code,
                row.wh_name,
                row.status,
    ...
    return (
        <div className="mt-8 space-y-4 pt-6 border-t border-gray-200 dark:border-gray-600">
             <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 text-center">
                Tiếp tục Bước 2: Nạp dữ liệu cho Bảng `warehouses`
            </h3>
            <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-2">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">Input: warehouses.json</h3>
...
export default DataSeederWarehouses;