    ...
    const fetchGoodsTypes = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('goods_types')
          .select('*')
          .order('goods_type_code', { ascending: true });
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {goodsTypes.map((gt) => (
                <tr key={gt.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{gt.goods_type_code}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{gt.goods_type_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{gt.description}</td>
